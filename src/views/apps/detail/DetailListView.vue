...
<template>
    <vx-card title="轮播新增" >
        <div class="mt-5">
            <form-wizard color="rgba(var(--vs-primary), 1)" :title="null" :subtitle="null" finishButtonText="更新"  @on-complete="formSubmitted">
...
export default {
  ...
  methods: {
    ...
    formSubmitted () {
      this.$vs.loading()
      console.log(this.id)
      projectAPI.updateDetail({id: this.id, fileName: this.fileName, detail_title: this.detail_title, detail_type : this.detail_type_value, type_id:this.type_id})
        .then(response => { 
  ...
  created () {
    ...
    const detail = this.$store.state.project.detailList.find(val => {
      return val.id === this.$route.params.detailId
    })
    console.log(detail.detail_type)
