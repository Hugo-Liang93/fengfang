        ...
        <div class="flex-grow">
          <vs-dropdown vs-trigger-click class="cursor-pointer">
            <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
              <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ commentsData.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : commentsData.length }} of {{ commentsData.length }}</span>
...
// Cell Renderer
import CellRendererCommentLink from './cell-renderer/CellRendererCommentLink.vue'
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          filter: true,
          width: 225,
          cellRendererFramework: 'CellRendererCommentLink'
        },
        {
          headerName: '评论内容',
          field: 'like_comment',
  ...
  methods: {
    updateCommentStatus (Statustype) {
      ...
      activityAPI.updateCommentStatus(Statustype, {'passLs': ls.toString()}).then(() => {
  ...
  async created () {
    // 获取用户信息存到state中
    await activityAPI.getAllComment().then(response => {
      this.commentsData = response.data
    })
  }
}
