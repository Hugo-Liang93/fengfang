    ...
    <vx-card title="项目资料上传">
      ...
      <div class="vx-row">
        ...
        <template v-if="projectNameAble">
          ...
          <div class="mt-5">
              <vs-upload :action="`/upload/houseType/${project_name}`" fileName="file" text="户型图/区位图" @on-success="successUpload" />
          </div>
          ...
          <div class="mt-5">
              <vs-upload :action="`/upload/attach/${project_name}`" fileName="file" text="项目附件" @on-success="successUpload" />
          </div>
        </template>
    </vx-card>
</template>
